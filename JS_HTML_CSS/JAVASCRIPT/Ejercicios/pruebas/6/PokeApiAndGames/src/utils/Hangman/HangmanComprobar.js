...
export const hangmanComprobar = (inputValue, palabra) => {
  ...
  if (palabra[1].includes(minus)) {
    ...
    // Recorremos palabra random (en formato array)    //* La solución es verificar si el elemento ya ha sido acertado antes de intentar pintar la letra */
    const p = document.getElementById(`${minus}`);   //* intenta encontrar el elemento con el mismo id que ya ha sido cambiado Y por eso daba un error */
     palabra[0].forEach((element) => {
      // si la letra introducida coincide con alguna letra de la palabra
      if (p && !p.classList.contains("hangmanPacertado")) {
    ...
    if (pElementAcertados.length === palabra[0].length) {
      setTimeout(() => {
        ...
        h1.innerHTML = "Has capturado MR.MIME";
    ...
    if (contador === 6) {
      ...
      setTimeout(() => {
        ...
        div.innerHTML = "";
        const h1 = document.createElement("h1");
        h1.innerHTML = "MR.Mime te a engañado y se a ido corriendo";
        const button = document.createElement("button");
        button.innerHTML = "Play again";
        button.setAttribute("id", "hangmanReset");
        div.append(h1, button);
